  ...
  module Security
    class Enforcer
      ...
      def enforce(args, &blk)
        ...
        actions.each { |action| @constraints << Grant::Security::Constraint.new(action.to_sym, &blk) }
        associations.each_pair do |action, attributes|
          Array(attributes).each { |attr| @constraints << Grant::Security::Constraint.new(action.to_sym, attr, &blk) }
        end
      end
