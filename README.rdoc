...
To enable model attribute security you simply include the Grant::ModelAttributeSecurity module in your model class. In the example below you see two grant statements. The first grants permission to update the title attribute all the time. The second example grants permission to update list_in_store and price when the passed block evaluates to true, which in this case happens when the model is editable by the current user. A Grant::Error is raised if any grant block evaluates to false or nil.

  class Book < ActiveRecord::Base
    include Grant::ModelAttributeSecurity
    
    has_many :tags
    grant_attributes(:title) { true }
    grant_attributes(:list_in_store, :price) { |user, model| model.owned_by_user? user }
...
You can see what attributes are granted for the current user to edit by using model.granted_attributes, this is useful in situations where you're using something like formtastic and want to make the form display only fields the user is granted to edit. You can use granted_attributes without arguments to get all granted attributes, or you can pass it a list of attributes to check, in which case it will return an array of granted attributes limited to those passed. You can also pass :granted => false to get all ungranted attributes, or a subset based on those passed in:

  <%= semantic_form_for book do |f| %>
    <%= f.inputs *book.granted_attributes(:title, :list_in_store, :price) %>

    <% ungranted = book.granted_attributes(:title, :list_in_store, :price, :granted => false) %>
...
You can test whether a set of attributes is granted for the current user by using model.granted_attributes? with a list of attributes to test against:

  <% if book.granted_attributes?(:title) %>
  ...
  <% if book.granted_attributes?(:list_in_store, :price) %>
...
It is also possible to use model security and model attribute security in conjunction. The example below grants any user the ability to find and create, but only owners and administrators to update or destroy. It also grants any user the ability to update user_id when the book is a new record, but only administrators if it is not new. It grants anyone the ability to update :title and :price if the record is new or they own it, but it only allows administrators to update the list_in_store attribute
  ...
  class Book < ActiveRecord::Base
    ...
    grant_attributes(:title, :price) { |user, model| model.new_record? || model.owned_by_user? user }
    grant_attributes(:user_id) { |user, model| model.new_record? || user.administrator? }
    grant_attributes(:list_in_store) { |user, model| user.administrator? }
