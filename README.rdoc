...
You can see what attributes are granted for the current user to edit by using model.granted_attributes, useful in situations where you're using something like formtastic and want to make the form display only fields the user is granted to edit:

  <%= semantic_form_for book do |f| %>
    <%= f.inputs book.granted_attributes %>
  <% end %>
